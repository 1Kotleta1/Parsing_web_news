...
RG_NEWS_BASE_URL = "https://rg.ru/news.html"
RG_FEED_URL = "https://rg.ru/xml/index.xml"
FIVE_TV_NEWS_BASE_URL = "https://www.5-tv.ru/news/"
FIVE_TV_WORLD_LIST_URL = "https://www.5-tv.ru/news/list/world/"
RBC_RSSHUB_FEEDS = [
...
def is_technical_placeholder_text(value: str) -> bool:
...
def extract_article_text(html: str) -> tuple[str, str, str]:
    ...
    for selector in selectors:
        ...
        for node in nodes:
            ...
            if len(text) >= 40 and not is_technical_placeholder_text(text):
                chunks.append(text)
        if chunks:
            break

    content = "\n\n".join(chunks).strip()
    if is_technical_placeholder_text(content):
        content = ""
    if not content:
        desc = (
...
def fetch_rg_feed_items(
    ...
    *,
    ...
    seen: set[str] = set()
    try:
        xml_text = fetch_html(session, RG_FEED_URL, "RG-FEED")
        root = ET.fromstring(xml_text)
        stats["feed_ok"] = True
    except Exception as exc:
        stats["error"] = str(exc)
    ...
    for item in items:
        ...
        records.append(
            {
                "title": (item.findtext("title") or "").strip() or link.rsplit("/", 1)[-1],
                "url": link,
                "publishDate": (item.findtext("pubDate") or "").strip(),
                "body": extract_text_from_html_fragment((item.findtext("description") or "").strip()),
            }
        )
        stats["items_added"] += 1
    return records, stats
...
def parse_rg_source(
    *,
    ...
    records, feed_stats = fetch_rg_feed_items(session, max_items=max_items, deadline_ts=deadline_ts)
    stats["feed"] = feed_stats
    stats["entry_ok"] = bool(feed_stats.get("feed_ok"))
    stats["links_found"] = len(records)
    ...
    for idx, rec in enumerate(records, start=1):
        ...
        fallback_title = rec.get("title") or link.rsplit("/", 1)[-1]
        fallback_content = rec.get("body") or fallback_title or link
        fallback_dt_raw = rec.get("publishDate", "")
        fallback_dt = fallback_dt_raw
        if fallback_dt_raw:
            try:
                parsed = parsedate_to_datetime(fallback_dt_raw)
                if parsed.tzinfo is None:
                    parsed = parsed.replace(tzinfo=timezone.utc)
                fallback_dt = parsed.isoformat()
            except Exception:
                pass
        ...
        if idx > max(0, fulltext_limit):
            ...
            out.append(
                NewsItem(
                    ...
                    content=fallback_content,
                    source="RG news",
                    published_at=normalize_datetime_or_now(fallback_dt, link),
                )
            )
            stats["articles_success"] += 1
            continue
        ...
        try:
            stats["fulltext_attempted"] += 1
            html = fetch_html(session, link, "RG")
            title, content, page_dt = extract_article_text(html)
            if not title:
                title = fallback_title
            if not content:
            ...
            out.append(
                NewsItem(
                    ...
                    content=content,
                    source="RG news",
                    published_at=normalize_datetime_or_now(page_dt or fallback_dt, link),
                )
            )
            stats["articles_success"] += 1
        except Exception as exc:
            stats["articles_failed"] += 1
            logging.warning("[RG] Fulltext unavailable, fallback used: %s (%s)", link, exc)
            out.append(
                NewsItem(
                    title=fallback_title,
                    url=link,
    ...
    *,
    ...
    stats = {
        ...
        "link_extract": {
            ...
            "filtered_duplicates": 0,
            "entry_pages_ok": 0,
            "entry_pages_failed": 0,
        },
    }

    entry_pages = [FIVE_TV_NEWS_BASE_URL, FIVE_TV_WORLD_LIST_URL]
    links: list[str] = []
    seen: set[str] = set()
    include_patterns = [r"^/news/\d+/"]
    exclude_patterns = [r"^/news/?$", r"^/news/list/"]

    for entry_url in entry_pages:
        if is_deadline_reached(deadline_ts) or len(links) >= max_items:
            break
        try:
            html = fetch_html(session, entry_url, "5TV")
            stats["entry_ok"] = True
            stats["link_extract"]["entry_pages_ok"] += 1
        except Exception as exc:
            stats["link_extract"]["entry_pages_failed"] += 1
            logging.warning("[5TV] Entry page unavailable: %s (%s)", entry_url, exc)
            continue

        page_links, page_stats = extract_links_by_patterns(
            html=html,
            base_url=entry_url,
            max_items=max_items,
            include_patterns=include_patterns,
        ...
        for key in ("anchors_total", "filtered_external", "filtered_non_article", "filtered_duplicates"):
            stats["link_extract"][key] += page_stats.get(key, 0)
        for link in page_links:
            if len(links) >= max_items:
                break
            if link in seen:
                stats["link_extract"]["filtered_duplicates"] += 1
                continue
            seen.add(link)
            links.append(link)

    stats["links_found"] = len(links)
    logging.info("[5TV] Extracted links from entry pages: %s", len(links))

    out: list[NewsItem] = []
    for idx, link in enumerate(links, start=1):
        if is_deadline_reached(deadline_ts):
            logging.warning("[5TV] Stop by runtime limit while parsing articles.")
            break
        if idx > max(0, fulltext_limit):
            stats["fulltext_skipped"] += 1
            out.append(
        ...
        try:
            stats["fulltext_attempted"] += 1
            html = fetch_html(session, link, "5TV")
            title, content, page_dt = extract_article_text(html)
            if not title:
                title = link.rsplit("/", 1)[-1]
            if not content:
            ...
            stats["articles_failed"] += 1
            logging.exception("[5TV] Article parse failed: %s (%s)", link, exc)
...
def is_low_quality_item(item: NewsItem) -> bool:
...
def run(
    *,
    ...
    rg_max_items: int,
    rg_fulltext_limit: int,
    five_tv_max_items: int,
    five_tv_fulltext_limit: int,
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        rg_future = executor.submit(
            parse_rg_source,
            max_items=rg_max_items,
            fulltext_limit=rg_fulltext_limit,
            deadline_ts=deadline_ts,
        )
        five_tv_future = executor.submit(
            parse_5tv_source,
            max_items=five_tv_max_items,
            fulltext_limit=five_tv_fulltext_limit,
            deadline_ts=deadline_ts,
        )
        ...
        rg_items, rg_stats = safe_source_result("RG", rg_future)
        five_tv_items, five_tv_stats = safe_source_result("5TV", five_tv_future)
    ...
    payload = {
        ...
        "stats": {
            ...
            "combined": {
                ...
                "after_quality_filter": len(cleaned),
            },
        },
        "items": [asdict(item) for item in cleaned],
    }
    out_file.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    logging.info("Saved JSON: %s", out_file)
    logging.info("Combined result count: %s", len(cleaned))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Parallel news parser: RBC short_news + RIA + Lenta + Tproger + REN + MK + M24 + Gazeta + RG + 5TV"
        )
    )
    ...
    parser.add_argument("--rg-max-items", type=int, default=200)
    parser.add_argument("--rg-fulltext-limit", type=int, default=80)
    parser.add_argument("--five-tv-max-items", type=int, default=200)
    parser.add_argument("--five-tv-fulltext-limit", type=int, default=80)
    parser.add_argument(
        "--max-search-seconds",
        type=int,
        default=300,
    ...
    logging.info(
        "Parser start. Sources: %s | %s | %s | %s | %s | %s | %s | %s | %s | %s",
        RBC_BASE_URL,
        RIA_BASE_URL,
        LENTA_BASE_URL,
        TPROGER_NEWS_BASE_URL,
    ...
    out_file, stats = run(
        ...
        rg_max_items=args.rg_max_items,
        rg_fulltext_limit=args.rg_fulltext_limit,
        five_tv_max_items=args.five_tv_max_items,
        five_tv_fulltext_limit=args.five_tv_fulltext_limit,
